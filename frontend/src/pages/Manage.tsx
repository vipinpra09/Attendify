...
const BRANCHES = ["CSE", "CSE-AIML", "IT", "ECE", "EE", "Mechanical", "Civil"];
const DEPARTMENTS = ["CSE", "CSE-AIML", "IT", "ECE", "EE", "Mechanical", "Civil", "ASH", "ME", "Mathematics", "Physics"];
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Section" required error={errors.section}>
            <Select value={f.section} onChange={(e) => set("section", e.target.value)}>{SECTIONS.map((s) => <option key={s} value={s}>{s}</option>)}</Select>
          </Field>
    ...
    <Layout title="Students" sub="Manage student records and login accounts">
      ...
      <Card className="overflow-hidden">
        <div className="flex flex-wrap items-center gap-3 px-4 py-3.5 border-b border-slate-100">
          ...
          <Select value={sectionFilter} onChange={(e) => { setSectionFilter(e.target.value); setPage(1); }} className="w-auto min-w-[140px]">
            <option value="">All sections (A-G)</option>
...
/* =========================================================
   TEACHERS
   ========================================================= */
const TEACHER_LABELS: Record<string, string> = { Name: "name", Email: "email", Password: "password", Department: "department", Phone: "phone", "Employee code": "employeeCode" };
  ...
  return (
    <Modal open onClose={onClose} title={isEdit ? "Edit Teacher" : "Add Teacher"}
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <Field label="Employee Code" hint="e.g. 200914" error={errors.employeeCode}>
          <Input value={f.employeeCode} error={!!errors.employeeCode} onChange={(e) => set("employeeCode", e.target.value)} placeholder="200914" className="font-mono" />
        </Field>
...
export function TeachersPage() {
  ...
  const filtered = useMemo(() => {
    const q = search.toLowerCase();
    return (teachers ?? []).filter((t) => !q || t.name.toLowerCase().includes(q) || t.email.toLowerCase().includes(q) || t.department.toLowerCase().includes(q) || (t.employeeCode && t.employeeCode.toLowerCase().includes(q)));
  }, [teachers, search]);
  const paged = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
    ...
    <Layout title="Teachers" sub="Manage faculty records and subject assignments">
      ...
      <Card className="overflow-hidden">
        ...
        {loading ? <TableSkeleton rows={6} /> : paged.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="tbl">
              <thead><tr><th>ID</th><th>Emp Code</th><th>Name</th><th>Department</th><th>Phone</th><th>Subjects</th><th>Status</th><th className="text-right">Actions</th></tr></thead>
              <tbody>
                {paged.map((t, i) => (
                  <tr key={t.id} className="anim-fade-up" style={{ animationDelay: `${Math.min(i, 8) * 30}ms` }}>
                    ...
                    <td className="font-mono font-bold text-[12.5px] text-brand-700">{t.employeeCode || "—"}</td>
...
/* =========================================================
   SUBJECTS
   ========================================================= */
const SUBJECT_LABELS: Record<string, string> = { "Subject name": "name", "Subject code": "code", Department: "department", Semester: "semester", Teacher: "teacherId", "Subject type": "subjectType" };
...
function SubjectForm({ initial, isEdit, onClose, onSaved }: { initial: SubjectRow | null; isEdit: boolean; onClose: () => void; onSaved: (msg: string) => void }) {
  ...
  const [f, setF] = useState({
    ...
    subjectType: initial?.subjectType ?? "Theory",
  });
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [saving, setSaving] = useState(false);
  const set = (k: string, v: string | number) => setF((p) => ({ ...p, [k]: v }));
  ...
  return (
    <Modal open onClose={onClose} title={isEdit ? "Edit Subject" : "Add Subject"} subtitle="Assign the teacher who will take this subject"
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <Field label="Subject Code" required error={errors.code} hint="e.g. BCS 301"><Input value={f.code} error={!!errors.code} onChange={(e) => set("code", e.target.value)} placeholder="BCS 301" className="font-mono" /></Field>
        <Field label="Type" required error={errors.subjectType}>
          <Select value={f.subjectType} onChange={(e) => set("subjectType", e.target.value)}>
        ...
        <Field label="Teacher" required error={errors.teacherId}>
          <Select value={f.teacherId} error={!!errors.teacherId} onChange={(e) => set("teacherId", e.target.value)}>
            ...
            {(teachers ?? []).map((t) => <option key={t.id} value={t.id}>{t.name} — {t.department} {t.employeeCode ? `(${t.employeeCode})` : ""}</option>)}
          </Select>
        </Field>
        <Field label="Department" required error={errors.department}>
          <Select value={f.department} onChange={(e) => set("department", e.target.value)}>{DEPARTMENTS.map((d) => <option key={d}>{d}</option>)}</Select>
...
export function SubjectsPage() {
  ...
  const filtered = useMemo(() => {
    const q = search.toLowerCase();
    return (subjects ?? []).filter((s) => !q || s.name.toLowerCase().includes(q) || s.code.toLowerCase().includes(q) || s.teacherName.toLowerCase().includes(q) || (s.subjectType && s.subjectType.toLowerCase().includes(q)));
  }, [subjects, search]);
    ...
    <Layout title="Subjects" sub="Create subjects and assign teachers">
      ...
      <Card className="overflow-hidden">
        ...
        {loading ? <TableSkeleton rows={6} /> : filtered.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="tbl">
              <thead><tr><th>Code</th><th>Subject Name</th><th>Type</th><th>Department</th><th>Semester</th><th>Teacher</th><th className="text-right">Actions</th></tr></thead>
              <tbody>
                {filtered.map((s, i) => (
                  <tr key={s.id} className="anim-fade-up" style={{ animationDelay: `${Math.min(i, 8) * 30}ms` }}>
                    <td className="font-mono font-bold text-[12.5px] text-brand-700">{s.code}</td>
                    <td className="font-bold text-slate-700">{s.name}</td>
                    <td><Badge tone={s.subjectType === "Practical" ? "warn" : "brand"}>{s.subjectType || "Theory"}</Badge></td>
                    <td><Badge tone="slate">{s.department}</Badge></td>
                    <td className="font-semibold text-slate-600 tnum">Sem {s.semester}</td>
                    <td><span className="inline-flex items-center gap-2 font-semibold text-slate-600"><span className="grid place-items-center w-6.5 h-6.5 rounded-md bg-brand-50 text-brand-700 text-[10.5px] font-bold">{s.teacherName.split(" ").map((w) => w[0]).slice(0, 2).join("")}</span><span>{s.teacherName}{s.employeeCode ? <span className="text-slate-400 text-[11px] block">Emp: {s.employeeCode}</span> : null}</span></span></td>
                    <td>
                      <div className="flex justify-end gap-0.5">
                        <IconBtn title="Edit" onClick={() => setEditor({ isEdit: true, row: s })}><Pencil className="w-4 h-4" /></IconBtn>
                        <IconBtn title="Delete" tone="bad" onClick={() => setDeleting(s)}><Trash2 className="w-4 h-4" /></IconBtn>
