...
import { AlertTriangle, Bell, BookOpen, CalendarCheck2, CalendarX2, ChevronRight, Layers, RefreshCw, TrendingUp } from "lucide-react";
import { api } from "../lib/api";
import { useAuth } from "../lib/auth";
import { Layout } from "../components/layout";
import { Badge, Button, Card, EmptyState, Progress, PctBadge, Ring, StatCard, StatusBadge, cn } from "../components/ui";
import { TrendArea } from "../components/charts";
import { classesNeeded, fmtDate, fmtDay, isLow, MIN_ATTENDANCE, type Notice, type StudentStats } from "../lib/types";
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    void load();
    api.notices.list(token).then(setNotices).catch(() => undefined);
  }, [load, token]);
    ...
    <Layout title="My Dashboard" sub="Your personal attendance record — computed live, never typed in">
      ...
      {stats && !loading && (
        <div className="space-y-5">
          ...
          {notices.length > 0 && (
            <Card className="anim-fade-up p-5 border-brand-200/80 bg-gradient-to-r from-brand-50/40 via-white to-white">
              <div className="flex items-center justify-between gap-3 mb-3">
                <div className="flex items-center gap-2">
                  <span className="grid place-items-center w-8 h-8 rounded-lg bg-brand-100 text-brand-700">
                  ...
                  <div>
                    ...
                    <p className="text-[12px] text-slate-500">
                      You have {notices.length} active notice{notices.length === 1 ? "" : "s"}
                    </p>
                  </div>
                </div>
                <Link
                  to="/student/notices"
                  className="inline-flex items-center gap-1 text-[12.5px] font-bold text-brand-600 hover:text-brand-700 hover:underline"
                >
              ...
              <div className="space-y-2">
                {notices.slice(0, 2).map((n) => {
                  const isDirect = n.targetType === "STUDENT";
                  return (
                    <div
                      key={n.id}
                      className={cn(
                        ...
                        isDirect
                          ? "bg-ok-50/50 border-ok-200"
                          : n.priority === "URGENT"
                          ? "bg-bad-50/40 border-bad-200"
                    ...
                    >
                      <div className="flex items-center justify-between gap-2 mb-1 flex-wrap">
                        <div className="flex items-center gap-1.5">
                          {n.priority === "URGENT" && (
                            <Badge tone="bad" className="text-[10px] py-0">URGENT</Badge>
                          )}
                          {isDirect ? (
                            <Badge tone="ok" className="text-[10px] py-0">DIRECT TO YOU</Badge>
                          ) : n.targetType === "CLASS" ? (
                            <Badge tone="brand" className="text-[10px] py-0">CLASS NOTICE</Badge>
                          ) : (
                            <Badge tone="slate" className="text-[10px] py-0">ALL</Badge>
                          )}
                          <span className="font-bold text-[13.5px] text-night-900">{n.title}</span>
                        </div>
                        <span className="text-[11px] text-slate-400 font-medium">
                          {fmtDate(n.createdAt ? n.createdAt.slice(0, 10) : "")}
                        </span>
                      </div>
                        ...
                        <tr key={s.subjectId} className="anim-fade-up" style={{ animationDelay: `${i * 40}ms` }}>
                          <td>
                            <div className="flex items-center gap-2">
                              ...
                              {s.subjectType && (
                                <Badge tone={s.subjectType === "Practical" ? "warn" : "brand"} className="text-[10px] py-0 px-1.5 font-semibold">
                                  {s.subjectType}
                                </Badge>
                              )}
                            </div>
                            <p className="font-mono text-[11px] text-slate-400">{s.code} · {s.teacherName}{s.employeeCode ? ` (${s.employeeCode})` : ""}</p>
                          </td>
